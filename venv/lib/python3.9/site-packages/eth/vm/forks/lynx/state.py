...
from eth.vm.forks.frontier.constants import (
    REFUND_SELFDESTRUCT,
    MAX_REFUND_QUOTIENT,
)
from .transaction_context import (
    LynxTransactionContext
)
from .validation import validate_lynx_transaction


class LynxTransactionExecutor(BaseTransactionExecutor):
...
class LynxState(BaseState):
    computation_class: Type[ComputationAPI] = LynxComputation
    transaction_context_class: Type[TransactionContextAPI] = LynxTransactionContext
    ...
    def validate_transaction(self, transaction: SignedTransactionAPI) -> None:
        validate_lynx_transaction(self, transaction)